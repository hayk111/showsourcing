...
xdescribe("test dev environment variables, in case someone change those", () => {
	it("it should have the correct values", () => {
		expect(environment.production).toBe(false);
		expect(environment.graphqlUrl).toBe(
			"wss://showsourcingdev.us1.cloud.realm.io/graphql"
		);
		expect(environment.graphqlAuthUrl).toBe(
			"https://showsourcingdev.us1.cloud.realm.io/auth"
		);
