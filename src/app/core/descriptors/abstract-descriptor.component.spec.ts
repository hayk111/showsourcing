...
export class TestDescriptor extends AbstractDescriptorComponent {
	...
	sample: DynamicField = { name: 'sample', type: 'boolean', label: 'Sample available' };
...
describe('EntityDescriptors', () => {
	...
	it('should modify the descriptor, given a set of DynamicField for any item on this set has matching key ("name")', () => {
		const newName = { ...testDesc.name, metadata: { placeholder: 'I didnt have placeholder before' } };
		const newSample = { ...testDesc.sample, label: 'new sample label' };
		const newCategory = { ...testDesc.category, metadata: { ...testDesc.category.metadata, placeholder: 'choose new category' } };

		testDesc.modify([
			{ name: 'name', metadata: { placeholder: 'I didnt have placeholder before' } },
	...
	it('should insert a Dynamic field above a given key ("name") that exists on the descriptor', () => {
		...
		expect(testDesc.descriptor).toEqual([testDesc.name, newField, testDesc.sample, testDesc.category]);
	});
	...
	it('should throw error when inserting a Dynamic field if the Dynamic field does not have key ("name")', () => {
		...
		expect(() => testDesc.insert(newField, 'sample'))
			.toThrowError(Error);
	});
