...
export class DetailsPageComponent extends AutoUnsub implements OnInit {
	...
	update(supplier: Supplier) {
		api.col('Supplier').update([supplier as any]);
	}
	...
	delete(supplier: Supplier) {
		...
		.pipe(
			switchMap(_ => api.col('Supplier').delete([{ id: supplier.id }]).local$)
	...
	onArchive(supplier: Supplier | Supplier[]) {
		if (Array.isArray(supplier)) {
			...
			api.col('Supplier').update([updated as any]).local$
				.subscribe(_ => {
					this.toastSrv.add({
			...
			const { id } = supplier;
			api.col('Supplier').update([{ id, archived: true }]).local$
				.subscribe(_ => {
					this.toastSrv.add({
						type: ToastType.SUCCESS,
