...
export class DashboardService {
	...
	getCountProductsNeedReview(): Observable<number> {
		...
		return this.productSrv.queryCount('status == NULL && archived != false && deleted != false');
	}

	getCountProductsInWorkflow(): Observable<number> {
		return this.productSrv.queryCount('status.status.inWorkflow == true && archived != false && deleted != false');
	}

	getCountSupplierUnderAssessment(): Observable<number> {
		return this.supplierSrv.queryCount('status.status.name == "_UnderAssessment" && archived != false && deleted != false');
	}

	getCountTotalTasks(): Observable<number> {
		return this.taskSrv.queryCount('archived != false && deleted != false');
	}

	getCountTasksInProgress(): Observable<number> {
		return this.taskSrv.queryCount('status.status.name == "_InProgress" && archived != false && deleted != false');
	}

	getFirstFewTasks(): Observable<Task[]> {
		return this.taskSrv.queryMany({ take: 3 });
	}
}
