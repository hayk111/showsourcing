...
export class TokenEffects {
	...
	set$ = this.actions$.ofType<any>(TokenActionType.SET).pipe(
		map(action => action.payload),
		tap(token => this.srv.token = token)
	);

	@Effect()
	checkValid$ = this.actions$.ofType<any>(TokenActionType.CHECK).pipe(
	...
	@Effect()
	checkInvalid$ = this.actions$.ofType<any>(TokenActionType.CHECK).pipe(
		filter(_ => !this.srv.checkAuthToken()),
		map( _ => AuthActions.logout() )
	);

	@Effect({ dispatch: false })
	remove$ = this.actions$.ofType<any>(TokenActionType.REMOVE).pipe(
