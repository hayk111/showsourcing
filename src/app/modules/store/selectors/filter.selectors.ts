...
export const selectFilterForEntity = (filterGroupName: FilterGroupName, target: EntityRepresentation, fromRoot = true) =>
(state): Array<Filter> => {
	const groupFilters = fromRoot ? state.filters[filterGroupName] : state[filterGroupName];
	if (groupFilters === undefined)
		throw Error(`filterGroupName for ${filterGroupName} is undefined. ${r}`);
	const filtersForTarget = groupFilters.filters.filter(f  => f.target.entityName === target.entityName);
	return filtersForTarget;
};


export const selectFilterValuesForEntity = (filterGroupName: FilterGroupName, target: EntityRepresentation) =>
(state): Array<any> => {
	const filtersForCategory = selectFilterForTarget(filterGroupName, target)(state);
...
// export const selectActiveFiltersForTarget = (filterGroupName: FilterGroupName, target: FilterTarget) => {
// 	const valsFilteredSelector = selectFilterValuesForTarget(filterGroupName, target);
