...
import { ActionType } from '../../action/misc/entity-mapper';
...
export class EntityMapperEffects {
	...
	@Effect()
	loadForSelection$ = this.actions$.ofType(ActionType.LOAD_FOR_SELECTION).pipe(
		map((action: TypedAction<any>) => action.payload),
		// adding target to the payload. Target is the currently selected item. For example if we
		// are on the product/128 page the target item will be the product with id 128.
		withLatestFrom(this.selectionSrv.getSelection(), ( toLoad, target ) => ({toLoad, target}) ),
		switchMap(
			(p: { toLoad: any, target: any}) => this.srv.loadForTarget(p.toLoad, p.target),
			((p: any, r: any) => this.mapLoadForTarget(p, r))
		)
	);

  // @Effect()
  // addForTarget$ = this.actions$.ofType(ActionType).pipe(
  //   map((action: TypedAction<any>) => action.payload),
  //
  // )
