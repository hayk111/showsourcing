...
import { SupplierStatusService } from '~core/erm/services/supplier-status/supplier-status.service';
import { ListPageService } from '~core/list-page';
import { SupplierService } from '~core/erm/services';
import { EntityName, ERM, Supplier, SupplierStatus } from '~core/erm/models';
...
export class SuppliersBoardComponent extends AutoUnsub implements OnInit {
	...
	ngOnInit() {
		...
		combineLatest(
			...
			mergeMap(([filterList, statuses]) => combineLatest(...this.getSupplierColumns(statuses, filterList))),
	...
	loadMore(col: KanbanColumn) {
		...
		this.supplierSrv.selectMany({
			...
			.subscribe(suppliers => this.kanbanSrv.setData([{ id: col.id, data: suppliers }]));
	}

	private getSupplierColumns(statuses: SupplierStatus[], filterList: FilterList) {
