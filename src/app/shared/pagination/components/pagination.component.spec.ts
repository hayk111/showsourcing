...
xdescribe("Component: PaginationComponent", () => {
	...
	beforeEach(() => {
		...
		previousBtn = fixture.nativeElement.querySelector(
			'icon-app[name="angle-left"]'
		);
	...
	it("clicking on next icon should go to next page", () => {
		...
		expect(paginationComp.goToPage.emit).toHaveBeenCalledWith(
			paginationComp.currentPage
		);
	});
