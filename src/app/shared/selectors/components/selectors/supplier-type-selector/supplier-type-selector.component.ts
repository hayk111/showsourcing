...
import { SupplierTypeService } from '~core/entity-services';
import { SupplierType, ERM } from '~core/models';
...
@Component({
	selector: 'supplier-type-selector-app',
	templateUrl: './supplier-type-selector.component.html',
	styleUrls: ['./supplier-type-selector.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class SupplierTypeSelectorComponent extends AbstractSelectorComponent {
	...
	constructor(
		protected supplierTypeSrv: SupplierTypeService,
	...
	setup() {
		this.config = {
			entitySrv: this.supplierTypeSrv,
			entityMetadata: ERM.SUPPLIER_TYPE,
	...
	createFn(): any {
		...
		let item;
		let createObs$: Observable<SupplierType>;
		if (name) {
			item = new SupplierType({ name });
			createObs$ = this.supplierTypeSrv.create(item);
		}
