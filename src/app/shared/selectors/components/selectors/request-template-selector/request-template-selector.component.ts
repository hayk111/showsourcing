...
@Component({
	selector: 'request-template-selector-app',
	templateUrl: './request-template-selector.component.html',
	styleUrls: ['./request-template-selector.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class RequestTemplateSelectorComponent extends AbstractSelectorComponent {
	...
	constructor(
		protected requestTemplateSrv: RequestTemplateService,
	...
	setup() {
		this.config = {
			entitySrv: this.requestTemplateSrv,
			entityMetadata: ERM.REQUEST_TEMPLATE,
	...
	createFn(): any {
		...
		let item;
		let createObs$: Observable<RequestTemplate>;
		if (name) {
			item = new RequestTemplate({ name });
			createObs$ = this.requestTemplateSrv.create(item);
		}
