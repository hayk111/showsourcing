...
@Component({
	template: `
		...
		<br>
		<div style="visibility: hidden" notifBadge [badge]="2" badgePosition="center" badgeColor="primary" badgeSize="m"></div>
		<br>
		<div style="visibility: hidden" notifBadge [badge]="3" badgePosition="no-content"></div>
		<br>
		<div style="visibility: hidden" notifBadge badgePosition="above-before"></div>
		<br>
		<div style="visibility: hidden" notifBadge [badge]="5" badgeHidden="true"></div>
		<br>
		<div style="visibility: hidden" notifBadge [badge]="6" badgeOverlap="true"></div>
		`
})
...
describe('NotificationBadgeDirective', () => {
	...
	beforeEach(async () => {
		TestBed.configureTestingModule({ declarations: [NotificationBadgeDirective, TestComponent] });
		fixture = TestBed.createComponent(TestComponent);
		component = fixture.componentInstance;
		debugEls = fixture.debugElement.queryAll(By.directive(NotificationBadgeDirective));
	...
	it('should contain "notif-badge-s" class by default', () => {
		const badgeDbgEl = debugEls[0];
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-s')).toBe(true, 'badge should contain "notif-badge-s" class');
	});

	it('should contain "notif-badge-above" and "notif-badge-after" class by default', () => {
		const badgeDbgEl = debugEls[0];
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-above'))
			.toBe(true, 'badge should have "notif-badge-above" class');
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-after'))
	...
	it('should display correct value', () => {
		const badgeDbgEl = debugEls[0];
		directive = badgeDbgEl.injector.get(NotificationBadgeDirective);
		spyOnProperty(directive, 'badge', 'get').and.returnValue(component.value.toString());
		fixture.detectChanges();
	...
	it('should have default color "warn" and "bg-warn" class', () => {
		...
		expect(span.classList.contains('bg-warn'))
			.toBe(true, 'badge should have "bg-warn" class');
	});
	...
	it('should contain correct class', () => {
		...
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-center'))
			.toBe(true, 'should contain "notif-badge-center" class');
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-m'))
			.toBe(true, 'should contain "notif-badge-m" class');
		const span = badgeDbgEl.nativeElement.children[0];
		expect(span).toBeTruthy();
		expect(span.classList.contains('color-white'))
			.toBe(true, 'should contain "color-white" class');
		expect(span.classList.contains('notif-badge-content'))
			.toBe(true, 'should contain "notif-badge-content" class');
		expect(span.classList.contains('notif-badge-active'))
			.toBe(true, 'should contain "notif-badge-active" class');
	});

	// 3rd badge
	it('should contain "notif-badge-no-content" class when badgePosition is "no-content"', () => {
		const badgeDbgEl = debugEls[2];
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-no-content'))
	...
	it('should contain "notif-badge-overlap" class when badgeOverlap is true', () => {
		...
		expect(badgeDbgEl.nativeElement.classList.contains('notif-badge-overlap'))
