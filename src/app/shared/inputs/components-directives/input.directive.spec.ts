...
xdescribe("InputDirective", () => {
	...
	beforeEach(async () => {
		TestBed.configureTestingModule({
			declarations: [TestComponent, InputDirective]
		});
	...
	it("should setting cursor at the end of text when focus the element", () => {
		...
		expect(inp.selectionStart)
			.withContext(
				'"selectionStart" of input should as same as length of value input'
			)
			.toEqual(inp.value.length);
		expect(inp.selectionEnd)
			.withContext(
				'"selectionEnd" of input should as same as length of value input'
			)
			.toEqual(inp.value.length);
	});
	...
	it("should throw error when type is not supported", () => {
		...
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "button"')
			.toThrowError(Error);

		component.typeInp = "checkbox";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "checkbox"')
			.toThrowError(Error);

		component.typeInp = "file";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "file"')
			.toThrowError(Error);

		component.typeInp = "hidden";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "hidden"')
			.toThrowError(Error);

		component.typeInp = "image";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "image"')
			.toThrowError(Error);

		component.typeInp = "radio";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "radio"')
			.toThrowError(Error);

		component.typeInp = "range";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "range"')
			.toThrowError(Error);

		component.typeInp = "reset";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "reset"')
			.toThrowError(Error);

		component.typeInp = "submit";
		expect(() => fixture.detectChanges())
			.withContext('should throw error when type is "submit"')
			.toThrowError(Error);
	});
