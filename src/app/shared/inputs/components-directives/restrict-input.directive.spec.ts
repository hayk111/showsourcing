...
xdescribe("RestrictInputDirective", () => {
	...
	const decimalKeyNotAllow = [
		["x", 120],
		["/", 47],
		["[", 91],
		["z", 122]
	];
	const decimalKeyAllow = [
		["0", 48],
		["1", 49],
		["7", 55],
		[".", 46],
	...
	const testInp = (type, allowArray, notAllowArray) => {
		...
		for (const item of allowArray) {
			...
			expect(directive.onKeyPress)
				.withContext(
					`Should call onKeyPress; key: ${item[0]} - charCode: ${item[1]}`
				)
				.toHaveBeenCalled();
