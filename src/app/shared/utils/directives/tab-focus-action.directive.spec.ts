...
xdescribe("TabFocusActionDirective", () => {
	...
	beforeEach(async () => {
		TestBed.configureTestingModule({
			declarations: [TestComponent, TabFocusActionDirective]
		});
	...
	it("should call output typing when typing characters or space", async () => {
		...
		subscriptions.add(
			directive.typing.subscribe(res => {
				if (event.key === "Q") {
					expect(res)
						.withContext(
							'should call output typing when typing characters (expect key "Q"'
						)
						.toBe("Q");
				} else if (event.key === " ") {
					expect(res)
						.withContext(
							'should call output typing when typing spac (expect key " "'
						)
						.toBe(" ");
				}
			})
	...
	it("should call output onKeydownEnter when enter", () => {
		...
		subscriptions.add(
			directive.keyEnter.subscribe(res =>
				expect(res)
					.withContext(
						'should call output typing when typing space (expect key " "'
					)
					.toBeFalsy()
			)
		);
