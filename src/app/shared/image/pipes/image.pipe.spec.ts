...
import { AppImage, Supplier } from '~core/orm/models';
import {
...
describe('ImagePipe', () => {
	...
	it(`transform (supplierWithoutImage) to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage)).toBe(`${DEFAULT_IMG}`);
	});

	it(`transform (supplierWithoutImage, "xs") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, 'xs')).toBe(`${DEFAULT_IMG}`);
	});

	it(`transform (supplierWithoutImage, "s") to"${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, 's')).toBe(`${DEFAULT_IMG}`);
	});

	it(`transform (supplierWithoutImage, "m") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, 'm')).toBe(`${DEFAULT_IMG}`);
	});

	it(`transform (supplierWithoutImage, "l") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, 'l')).toBe(`${DEFAULT_IMG}`);
	});

	it(`transform (supplierWithoutImage, "xl") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, 'xl')).toBe(`${DEFAULT_IMG}`);
	});
