...
xdescribe("ImagePipe", () => {
	...
	beforeEach(async () => {
		TestBed.configureTestingModule({
			declarations: [ImagePipe, TestComponent]
		});
	...
	it(`transforms (undefined, undefined,"category") to "${DEFAULT_CATEGORY_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "category")).toBe(
			"assets/icons/category.svg"
		);
	});

	it(`transforms (undefined, undefined, "event") to "${DEFAULT_EVENT_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "event")).toBe(
			"assets/icons/event.svg"
		);
	});

	it(`transforms (undefined, undefined, "product") to "${DEFAULT_PRODUCT_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "product")).toBe(
			"assets/icons/product.svg"
		);
	});

	it(`transforms (undefined, undefined, "product-list") to "${DEFAULT_PRODUCT_LIST_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "product-list")).toBe(
			"assets/icons/product-list.svg"
		);
	});

	it(`transforms (undefined, undefined, "project") to "${DEFAULT_PROJECT_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "project")).toBe(
			"assets/icons/project.svg"
		);
	});

	it(`transforms (undefined, undefined, "request") to "${DEFAULT_REQUEST_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "request")).toBe(
			"assets/icons/request-list.svg"
		);
	});
	...
	it(`transforms (undefined, undefined, "user") to "${DEFAULT_USER_ICON}"`, () => {
		expect(component.pipe.transform(undefined, undefined, "user")).toBe(
	...
	it(`transforms (undefined, undefined, undefined) to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(undefined, undefined, undefined)).toBe(
	...
	it(`transform (supplierWithImage) to "${baseUrl}/xl/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage)).toContain(
			`${baseUrl}/xl`
		);
	});

	it(`transform (supplierWithImage, "xs") to "${baseUrl}/xs/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage, "xs")).toContain(
			`${baseUrl}/xs`
		);
	});

	it(`transform (supplierWithImage, "s") to "${baseUrl}/s/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage, "s")).toContain(
			`${baseUrl}/s`
		);
	});

	it(`transform (supplierWithImage, "m") to "${baseUrl}/m/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage, "m")).toContain(
			`${baseUrl}/m`
		);
	});

	it(`transform (supplierWithImage, "l") to "${baseUrl}/l/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage, "l")).toContain(
			`${baseUrl}/l`
		);
	});

	it(`transform (supplierWithImage, "xl") to "${baseUrl}/xl/fileName"`, () => {
		expect(component.pipe.transform(supplierWithImage, "xl")).toContain(
			`${baseUrl}/xl`
		);
	});

	// Supplier, product, Entity object... - supplierWithoutImage  -> should check if (!value.images) return this.getDefault(type);
	it(`transform (supplierWithoutImage) to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage)).toBe(
			`${DEFAULT_IMG}`
		);
	});

	it(`transform (supplierWithoutImage, "xs") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, "xs")).toBe(
			`${DEFAULT_IMG}`
		);
	});

	it(`transform (supplierWithoutImage, "s") to"${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, "s")).toBe(
			`${DEFAULT_IMG}`
		);
	});

	it(`transform (supplierWithoutImage, "m") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, "m")).toBe(
			`${DEFAULT_IMG}`
		);
	});

	it(`transform (supplierWithoutImage, "l") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, "l")).toBe(
			`${DEFAULT_IMG}`
		);
	});

	it(`transform (supplierWithoutImage, "xl") to "${DEFAULT_IMG}"`, () => {
		expect(component.pipe.transform(supplierWithoutImage, "xl")).toBe(
			`${DEFAULT_IMG}`
		);
	});
	...
	it(`transform ("${fileName}") to "${baseUrl}/xl/${fileName}"`, () => {
		expect(component.pipe.transform(fileName)).toBe(
			`${baseUrl}/xl/${fileName}`
		);
	});

	it(`transform ("${fileName}", "xs") to "${baseUrl}/xs/${fileName}"`, () => {
		// missing size xs for "ImageUrls"
		expect(component.pipe.transform(fileName, "xs")).toBe(
			`${baseUrl}/xs/${fileName}`
		);
	});

	it(`transform ("${fileName}", "s") to "${baseUrl}/s/${fileName}"`, () => {
		expect(component.pipe.transform(fileName, "s")).toBe(
			`${baseUrl}/s/${fileName}`
		);
	});

	it(`transform ("${fileName}", "m") to "${baseUrl}/m/${fileName}"`, () => {
		expect(component.pipe.transform(fileName, "m")).toBe(
			`${baseUrl}/m/${fileName}`
		);
	});

	it(`transform ("${fileName}", "l") to "${baseUrl}/l/${fileName}"`, () => {
		expect(component.pipe.transform(fileName, "l")).toBe(
			`${baseUrl}/l/${fileName}`
		);
	});

	it(`transform ("${fileName}", "xl") to "${baseUrl}/xl/${fileName}"`, () => {
		expect(component.pipe.transform(fileName, "xl")).toBe(
			`${baseUrl}/xl/${fileName}`
		);
	});
});
