export * from './product-big-card/product-big-card.component';
export * from './product-info-card/product-info-card.component';
export * from './product-files/product-files.component';
export * from './product-list-view/product-list-view.component';
export * from './product-page/product-page.component';
export * from './product-side-preview/product-side-preview.component';
export * from './product-tasks/product-tasks.component';
export * from './products-page/products-page.component';
